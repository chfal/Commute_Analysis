---
title: 'Final Project: Is My Commute _Really_ a Nightmare?'
...
```{r, out.width='100%', echo=FALSE, fig.cap="Cleo's Commute, Mapped Out"}
...
p_value <- c(1.455e-07,.0006423,2.2e-12,.0009328,.03963,1.08e-10,5.959e-12,9.928e-06)
...
table_1 <- full_join(mean_time, stdev_time, by="segment")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.cap="Histograms of Different Segments of the Commute"}
...
commute_3 %>%
  ...
  labs(title = "Histograms of Different Commute Segments")
```
...
legend(x="topright",legend=c("gamma","normal", "weibull"), lty=1, col=c("blue", "red","green"))
```
...
going to school in the morning, or coming home at night?
...
plot_1 <- ggplot(mapping=aes(home_vector)) +
  geom_histogram(fill="darkorange2", binwidth = 3) +
  theme_bw() +
  labs(title="Total Commute Time Going Home", x="Time it Took to Go Home (minutes)")

plot_2 <- ggplot(mapping=aes(school_vector)) +
  geom_histogram(fill="darkturquoise", binwidth=3) +
  theme_bw() +
  labs(title="Total Commute Time Going to School", x="Time it Took to Get to School (minutes)")
```
...
plot(commute_2$green_line,commute_2$total_time, main = "effect of green line on total time", xlab="green line time", ylab="total time")
...
plot(commute_2$wait_green,commute_2$total_time, main = "effect of waiting for the green line on total time", xlab="waiting for green  time", ylab="total time")
